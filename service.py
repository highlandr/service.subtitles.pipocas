...
subtitle_pattern = "<a href=\"" + main_url + \
    "legendas/info/(.+?)\" class=\"text-dark no-decoration\">"
...
def enable_rar():
    ...
    if not is_rar_enabled():
        xbmc.executeJSONRPC(
            '{"jsonrpc": "2.0", "method": "Addons.SetAddonEnabled", "params": {"addonid": "vfs.libarchive", "enabled": true} }')
        time.sleep(1)
        if not is_rar_enabled():
            dialog = xbmcgui.Dialog()
            ok = dialog.ok(__language__(32012).encode(
                "utf-8"),  __language__(32013).encode("utf-8"), " ", __language__(32014).encode("utf-8"))
...
def getallsubs(searchstring, languageshort, languagelong, file_original_path, searchstring_notclean):
    ...
    if result.status_code == '403':
        xbmcgui.Dialog().notification(_scriptname, _language(
            32019).encode('utf8'), xbmcgui.NOTIFICATION_ERROR)
        return []
    ...
    page = 1
    if languageshort == "pt":
        url = main_url + "legendas?t=rel&l=portugues&page=" + \
            str(page) + "&s=" + urllib.quote_plus(searchstring)
    elif languageshort == "pb":
        url = main_url + "legendas?t=rel&l=brasileiro&page=" + \
            str(page) + "&s=" + urllib.quote_plus(searchstring)
    elif languageshort == "es":
        url = main_url + "legendas?t=rel&l=espanhol&page=" + \
            str(page) + "&s=" + urllib.quote_plus(searchstring)
    elif languageshort == "en":
        url = main_url + "legendas?t=rel&l=ingles&page=" + \
            str(page) + "&s=" + urllib.quote_plus(searchstring)
    else:
        url = main_url + "home"
...
def append_subtitle(item):
    ...
    listitem.setProperty("hearing_imp", 'true' if item.get(
        "hearing_imp", False) else 'false')
...
def Search(item):
    ...
    if username == '' or password == '':
        ...
        if username == '' and password != '':
            xbmcgui.Dialog().notification(_scriptname, _language(
                32016).encode('utf8'), xbmcgui.NOTIFICATION_ERROR)
        if username != '' and password == '':
            xbmcgui.Dialog().notification(_scriptname, _language(
                32017).encode('utf8'), xbmcgui.NOTIFICATION_ERROR)
        if username == '' and password == '':
            xbmcgui.Dialog().notification(_scriptname, _language(
                32018).encode('utf8'), xbmcgui.NOTIFICATION_ERROR)
    # PARENT FOLDER TWEAK DEFINED IN THE ADD-ON SETTINGS (AUTO | ALWAYS ON (DEACTIVATED) | OFF)
    ...
    if PT_ON == 'true':
        subtitles_list = getallsubs(
            searchstring, "pt", "Portuguese", file_original_path, searchstring_notclean)
        for sub in subtitles_list:
            append_subtitle(sub)
    if PTBR_ON == 'true':
        subtitles_list = getallsubs(
            searchstring, "pb", "Brazilian", file_original_path, searchstring_notclean)
        for sub in subtitles_list:
            append_subtitle(sub)
    if ES_ON == 'true':
        subtitles_list = getallsubs(
            searchstring, "es", "Spanish", file_original_path, searchstring_notclean)
        for sub in subtitles_list:
            append_subtitle(sub)
    if EN_ON == 'true':
        subtitles_list = getallsubs(
            searchstring, "en", "English", file_original_path, searchstring_notclean)
        for sub in subtitles_list:
            append_subtitle(sub)
    if PT_ON == 'false' and PTBR_ON == 'false' and ES_ON == 'false' and EN_ON == 'false':
        ...
        xbmcgui.Dialog().notification(_scriptname, normalizeString(
            'Apenas Português | Português Brasil | English | Spanish.'), xbmcgui.NOTIFICATION_ERROR)
...
def Download(id, filename):
    ...
    if content.content is not None:
        ...
        if packed:
            ...
            if len(subtitles) > 1:
                dialog = xbmcgui.Dialog()
                sel = dialog.select("FILES: %s" % filename, [
                                    y for x, y, z in subtitles])
                if sel >= 0:
...
if params['action'] == 'search' or params['action'] == 'manualsearch':
    ...
    if 'searchstring' in params:
        item['mansearch'] = True
        item['mansearchstr'] = urllib.unquote(
            params['searchstring']).decode('utf-8')
